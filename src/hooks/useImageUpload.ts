...
export interface UseImageUploadReturn {
  ...
  // Utilities
  getOptimizedUrl: (filename: string, options?: { width?: number; height?: number; quality?: number }) => string;
}
...
export const useImageUpload = (): UseImageUploadReturn => {
  ...
  const uploadImages = useCallback(async (files: File[]): Promise<UploadedImage[]> => {
    ...
    try {
      const { data, errors } = await StorageService.uploadImages(
        files,
        user.id,
        handleProgress
      );
  ...
  const compressAndUpload = useCallback(async (file: File): Promise<UploadedImage | null> => {
    if (!user) {
      setError('User not authenticated');
      toast.error('Please log in to upload images');
      return null;
  ...
  const getOptimizedUrl = useCallback((filename: string, options?: {
    width?: number;
    height?: number;
    quality?: number;
  }) => {
