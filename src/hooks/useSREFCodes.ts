...
export interface UseSREFCodesReturn {
  ...
  // Actions
  createSREFCode: (srefCode: Omit<SREFCodeInsert, 'user_id'>) => Promise<{ success: boolean; data?: SREFCode; error?: string }>;
  updateSREFCode: (codeId: string, updates: SREFCodeUpdate) => Promise<{ success: boolean; data?: SREFCode; error?: string }>;
...
export const useSREFCodes = (): UseSREFCodesReturn => {
  ...
  const getSREFCodeById = useCallback((codeId: string) => {
