...
import { Eye, EyeOff, Mail, Lock, Loader2, AlertCircle, Chrome, MessageSquare, Sparkles } from "lucide-react";
          ...
          <Card className="shadow-xl border-border/50 bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
            ...
            <CardContent className="space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" value={password} onChange={e => setPassword(e.target.value)} onBlur={() => setTouched({
                    ...touched,
                    password: true
                  })} className={cn("pl-10 pr-10 bg-background border-input focus:ring-2 focus:ring-ring transition-all", passwordError && touched.password && "border-destructive focus:ring-destructive/20")} disabled={isAnyLoading} aria-describedby={passwordError ? "password-error" : undefined} aria-invalid={!!passwordError && touched.password} />
                    <Tooltip>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Checkbox id="remember" checked={rememberMe} onCheckedChange={checked => setRememberMe(checked as boolean)} disabled={isAnyLoading} />
                    <Label htmlFor="remember" className="text-sm text-muted-foreground cursor-pointer">
                      Remember me
                    </Label>
                  </div>
                  <Button type="button" variant="link" onClick={handleForgotPassword} className="text-sm text-primary hover:text-primary/80 p-0 h-auto" disabled={isAnyLoading}>
                    Forgot password?
                  </Button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {/* Discord SSO */}
                <Button type="button" variant="outline" onClick={() => handleSsoLogin('discord')} disabled={isAnyLoading} className="bg-[#5865F2] hover:bg-[#4752C4] text-white border-[#5865F2] hover:border-[#4752C4] transition-all duration-200">
                ...
                {/* Google SSO */}
                <Button type="button" variant="outline" onClick={() => handleSsoLogin('google')} disabled={isAnyLoading} className="bg-white hover:bg-gray-50 text-gray-700 border-gray-300 hover:border-gray-400 transition-all duration-200">
              ...
              <div className="text-center pt-4 border-t border-border/50">
                <p className="text-sm text-muted-foreground">
                  ...
                  <Button type="button" variant="link" onClick={onSignUpClick} className="text-primary hover:text-primary/80 p-0 h-auto font-medium" disabled={isAnyLoading}>
                    Sign up
                  </Button>
                </p>
              </div>
