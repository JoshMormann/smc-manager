...
export default function SREFCodeCardGrid({
  ...
  // Loading state
  if (isLoading) {
    return <section className="p-6" aria-label="Loading SREF codes">
  ...
  // Error state
  if (error) {
    return <section className="p-6" aria-label="Error loading SREF codes">
  ...
  // Empty state
  if (codes.length === 0) {
    return <section className="p-6" aria-label="No SREF codes found">
  ...
  // Main grid
  return <section className="p-6" aria-label="SREF code collection">
      <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" role="list">
        {codes.map(code => <motion.li key={code.id} initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        duration: 0.3
            ...
            <Card className="overflow-hidden cursor-pointer hover:shadow-lg transition-shadow group focus-within:ring-2 focus-within:ring-ring" onClick={() => handleCardClick(code.code)} onKeyDown={e => handleKeyDown(e, code.code)} tabIndex={0} role="button" aria-label={`Copy SREF code for ${code.title}`}>
              <CardContent className="p-0">
                {/* Images */}
                <div className="grid grid-cols-3 gap-0" role="img" aria-label={`${code.title} reference images`}>
                  {code.images.map((image, index) => <figure key={index} className="aspect-square overflow-hidden">
                      <img src={image} alt={`${code.title} reference ${index + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                    </figure>)}
                </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <DropdownMenu>
                        <Tooltip delayDuration={0}>
                          <TooltipTrigger asChild>
                            <DropdownMenuTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100" onClick={e => e.stopPropagation()} aria-label={`More options for ${code.title}`}>
                        ...
                        <DropdownMenuContent align="end" className="w-32">
                          <DropdownMenuItem onClick={e => handleCardEdit(code.id, e)} className="cursor-pointer">
                            <Edit className="h-4 w-4 mr-2" />
                            Edit
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={e => handleCardDelete(code.id, e)} className="cursor-pointer text-destructive focus:text-destructive">
