...
export default function SidebarNavigation({
        ...
        <AnimatePresence>
          {folder.isExpanded && folder.children && (
            <motion.ul
              ...
              role="group"
              aria-label={`${folder.name} subfolders`}
            >
  ...
  return (
    <motion.aside
      ...
      className="bg-sidebar border-r border-sidebar-border flex flex-col shadow-lg"
      role="complementary"
    ...
    >
      ...
      <header className="p-4 border-b border-sidebar-border">
        <div className="flex items-center justify-between">
          {!collapsed && (
            <h2 className="text-lg font-semibold text-sidebar-foreground">SREF Manager</h2>
          )}
          <Tooltip delayDuration={0}>
            <TooltipTrigger asChild>
              <Button
                ...
                className="text-sidebar-foreground hover:bg-sidebar-accent focus:ring-2 focus:ring-ring"
      ...
      <ScrollArea className="flex-1 p-2">
        ...
        {activeTab === 'folders' && !collapsed && folders.length > 0 && (
          <section className="mt-6" aria-labelledby="folder-tree-heading">
        ...
        <Tooltip delayDuration={0}>
          <TooltipTrigger asChild>
            <button
              ...
              className={cn(
                'flex items-center w-full gap-3 p-3 rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-ring',
                'text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground',
                'border border-sidebar-border hover:border-sidebar-accent-foreground/20',
            ...
            >
              <Avatar className="h-8 w-8 border-2 border-sidebar-border shadow-sm">
                {profileAvatarUrl ? (
                  <AvatarImage src={profileAvatarUrl} alt={`${profileName}'s profile picture`} />
                ) : (
              ...
              {!collapsed && (
                <div className="flex flex-col items-start min-w-0">
                  ...
                  <span className="text-xs text-sidebar-foreground/70 truncate">{profileName}</span>
                </div>
              )}
            </button>
          </TooltipTrigger>
